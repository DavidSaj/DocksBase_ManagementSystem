...
export default function BerthPricingAssigner({ open, onClose, onNewRate, berthRates }) {
  ...
  const groups = useMemo(() => {
    ...
    filteredBerths.forEach(b => {
      const key = groupBy === 'pier' ? (b.pier_code || '—') : berthDisplayType(b);
      if (!map[key]) map[key] = [];
      map[key].push(b);
    });
  ...
  async function applyRate() {
    if (selected.size === 0 || !selectedRateId) return;
    setApplying(true);
    setApplyError('');
    try {
      await api.patch('/berths/bulk-pricing/', {
        berth_ids: [...selected],
        pricing_tier_id: selectedRateId === 'unassign' ? null : Number(selectedRateId),
      });
      await refetch();
      setSelected(new Set());
      setSelectedRateId('');
  ...
  const activeRates   = (berthRates ?? []).filter(r => r.is_active);
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 24px 12px', borderBottom: 'var(--border)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <div style={{ fontSize: 15, fontWeight: 700, flex: 1 }}>Assign Berth Rates</div>
                ...
                {isExpanded && (
                  <div>
                    ...
                    <div style={{
                      ...
                      {['', 'Code', 'Length', 'Current Rate'].map((h, i) => (
                        <div key={i} style={{
                    ...
                    {groupBerths.map(berth => (
                      ...
                      >
                        ...
                        <span style={{ fontSize: 12 }}>
                          {berth.pricing_tier_name
                            ? <span style={{ color: 'var(--navy)', fontWeight: 500 }}>{berth.pricing_tier_name}</span>
        ...
        {/* Action bar — extra bottom padding guards against taskbar overlap */}
        <div style={{
          ...
          {totalSelected === 0 ? (
            <span style={{ fontSize: 12, color: 'rgba(0,0,0,0.4)' }}>
              Select berths above to assign a rate
            </span>
          ) : (
            <>
              ...
              >
                <option value="">— Assign rate —</option>
                {activeRates.map(r => (
                  <option key={r.id} value={r.id}>{r.name}</option>
                ))}
                <option value="unassign">Remove rate (unassign)</option>
              </select>
              {onNewRate && (
                <button
                  type="button"
                  ...
                  title="Create a new berth rate in the catalog"
                >
                  <Ic n="plus" s={11} />
                  Custom Rate
                </button>
              ...
              <button
                ...
                disabled={!selectedRateId || applying}
                onClick={applyRate}
                style={{ flexShrink: 0 }}
              >
