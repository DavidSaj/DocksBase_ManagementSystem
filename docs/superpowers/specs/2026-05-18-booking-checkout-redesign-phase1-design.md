...
TTL is 24h from `created_at`. **File lifecycle on consumption:** inside the intent's atomic transaction, the file is *copied* from `MEDIA_ROOT/reservations/insurance/tmp/<token>.<ext>` to the `FileField` storage path generated by `upload_to`. A `transaction.on_commit` hook then deletes the source file in `/tmp/`. If the transaction rolls back (e.g. tetris fails on a later item), the `/tmp/` file is preserved and the boater can retry without re-uploading. The token row itself is kept for 30 days post-consumption for audit, then the row (but not the now-attached destination file) is purged.

**A single token may be referenced by multiple `ReservationItem` rows within the same intent request** (multi-boat shared insurance — see §5). The view stamps `consumed_at` once at the end of the transaction, not per-item.
...
- `insurance_upload_token`: looked up in `InsuranceUploadToken`. Must exist, belong to `request.tenant`, not have been consumed by a **prior** intent request, and be within TTL. The same token appearing on multiple items inside the *current* intent request is allowed (shared-policy case). `consumed_at` is stamped once at the end of the transaction. Otherwise `400`.
...
- Insurance certificate (`InsuranceUpload.jsx`):
  - **Boat 1:** file picker. On file selection, POSTs to `/public/reservations/insurance-upload/`, receives `{token, expires_at}`, stores `token` on `state.boats[0].insuranceToken`.
  - **Boat 2+:** "Use insurance from Boat 1" checkbox, **pre-checked**. While checked, the boat inherits `state.boats[0].insuranceToken` at submit time (the same token is included for this item). Unchecking the box reveals a file picker for an independent upload.
  - Required to advance iff `marina.requires_insurance_at_booking`. The shared-policy default means most multi-boat bookings only need one upload.
...
Existing Stripe `PaymentForm` extracted into its own file, with two changes to wire the collected billing data into Stripe properly:

1. **`PaymentElement` is configured to not collect address itself** (we already have it):
...
2. **`stripe.confirmPayment` passes our collected billing data via `payment_method_data.billing_details`** so AVS and Radar receive the address signals:
...
This keeps `PaymentElement` focused on the payment method itself (card number, wallets, etc.) while ensuring Stripe receives a complete `billing_details` payload for fraud scoring and AVS.
...
- **Primary deletion path: `transaction.on_commit`** at intent creation copies the file to its final `FileField` location and deletes the `/tmp/` source after the transaction commits.
- **Hourly Celery beat task `purge_expired_insurance_uploads`** (defensive backstop in case `on_commit` doesn't fire — process died between commit and hook):
  - For unconsumed tokens older than 24h: delete file, delete row.
  - For consumed tokens older than 30d: delete row.
  - **Also: for consumed tokens of any age, if the `/tmp/` source file still exists, delete it.** Catches the on_commit-didn't-fire edge case.
...
4. **Stripe `PaymentElement` billing address** — we ask Stripe NOT to collect it (`fields.billingDetails.address: 'never'`) and instead pass our own collected address via `confirmParams.payment_method_data.billing_details.address`. This preserves AVS coverage for US cards and keeps Radar's fraud-score address signals intact. If Stripe ever rejects our address shape, the fallback is to switch to `address: 'auto'` and let `PaymentElement` collect — we'd lose the data on our `Reservation` row but the flow would still work.
