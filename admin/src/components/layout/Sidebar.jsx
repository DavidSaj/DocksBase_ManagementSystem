  ...
  return (
    <aside className="sb">
      <div className="sb-logo">
        ...
        <div style={{ width: 1, height: 30, background: 'rgba(255,255,255,0.18)', flexShrink: 0, margin: '0 2px' }} />
        <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1, gap: 3 }}>
          ...
          <span style={{ fontFamily: 'var(--font-brand)', fontWeight: 300, fontSize: 11, letterSpacing: 3, color: '#c9a84c' }}>Base</span>
        </div>
      </div>

      <div className="sb-env">
        <div className="sb-env-label">Super Admin</div>
        <div className="sb-env-desc">Platform control panel</div>
      </div>
        ...
        <div key={group.group} className="sb-section">
          ...
          {group.items.map(item => (
            ...
            >
              ...
              {item.label}
              {counts?.[item.id] != null && (
                <span className={`sb-badge${item.alert ? ' alert' : ''}`}>{counts[item.id]}</span>
              )}
            </div>
          ))}
        </div>
