...
// Keep in sync with frontend/src/components/layout/Sidebar.jsx (FEATURE_DEFAULT_OFF).
const FEATURE_DEFAULT_OFF = new Set([
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {FEATURE_GROUPS.map(({ group, desc, features: groupFeatures }) => {
            const onCount = groupFeatures.filter(f => isFeatureEnabled(features, f.key)).length;
            ...
            <div className="card" key={group}>
              ...
              <div style={{ padding: '14px 18px', display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {groupFeatures.map(({ key, label }) => {
                  const effective = isFeatureEnabled(features, key);
                  return (
                    <FeatureChip
                      key={key}
                      label={label}
                      ...
                      onToggle={() => toggleFeature(key, !effective)}
                    />
                  );
                })}
              </div>
