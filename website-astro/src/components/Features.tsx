...
type Module = { title: string; desc: string; icon: ReactNode }
type Tab = { label: string; modules: Module[] }
...
const TABS: Tab[] = [
  {
    ...
    modules: [
      ...
      {
        title: 'Marina Map',
        desc: 'Interactive digital-twin harbor map — drag berths onto the canvas, color-coded by status, click any slip for vessel detail. Real-time, always accurate.',
      ...
      },
      {
        ...
        desc: 'Online self-service bookings powered by a mathematical gap-minimisation algorithm. Automatically assigns the most efficient available berth, or let your team do it manually.',
  ...
  },
  {
    ...
    modules: [
      ...
      {
        title: 'Staff & Rota',
        desc: 'Staff register, rota planning, time tracking, certifications, and skills matrix. Assign crew to jobs and get full accountability over who did what and when.',
    ...
    modules: [
      {
        ...
        desc: 'Auto-invoices on departure, utility meter billing, fuel dock POS, aged debtor chase workflow, and batch exports in CSV/PDF/XLSX.',
      ...
      {
        title: 'Sales',
        desc: 'Sales pipeline for new marina contracts, lead tracking, and conversion funnel — keep your commercial team organised alongside daily operations.',
  ...
  },
  {
    ...
    modules: [
      ...
      {
        title: 'Boater Portal',
        desc: 'White-label self-service portal for your boaters — view invoices, request crane lifts, report absence, and manage their own vessel details under your brand.',
  ...
  },
  {
    ...
    modules: [
      {
        title: 'Field App',
        desc: 'Full harbor management from any phone or tablet — no app download needed. Staff check reservations, log maintenance, update berth status, and handle crane requests right from the dock.',
      ...
      {
        title: 'Offline Mode',
        desc: 'Critical data cached locally — berth assignments, work orders, and arrivals list stay accessible even without a signal. Changes sync automatically when back online.',
        icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M1 6l11 11L23 6"/><path d="M1 1l22 22"/></svg>,
      },
      {
        title: 'Push Notifications',
        desc: 'Instant alerts for new bookings, overdue invoices, maintenance escalations, and arrival check-ins — delivered directly to staff phones wherever they are on site.',
...
const MODULE_COUNT = TABS.reduce((sum, t) => sum + t.modules.length, 0)
    ...
    <section className={styles.section} id="features">
      <div className={styles.inner}>
        <div className={styles.header}>
          ...
          <h2 className={styles.title}>{MODULE_COUNT} modules. One platform.</h2>
        ...
        <p className={styles.allModules}>
          <span className={styles.badge}>{MODULE_COUNT} modules included</span> — Operations · Yard & Crew · Finance · People · Mobile · Hospitality
        </p>
      </div>
    </section>
  )
